...
| `outageClass` | **Local** — the DISCOM's OMS "Down Info" value set |
...
| [`fault_reason_crosswalk.json`](fault_reason_crosswalk.json) | The DISCOM's FAULT_REASON master (FORM_ID 8312) → `cause.category`/`cause.subcategory` (IEEE 1782 §4.4/§4.5) crosswalk. |
...
| **`outageClass`** \* | `PLANNED` / `BREAKDOWN` / `SCHEDULED_ROSTERING` / `EMERGENCY_ROSTERING` | Outage class, from a DISCOM OMS "Down Info" set (local, additive enum). Rostering = rotational load-shedding (scheduled vs emergency). Restoration is a status transition (status=RESTORED), not a class. |
...
| `code` | text | Vendor/OMS fault-reason code, carried verbatim (e.g. a DISCOM's FAULT_REASON) — not standardized; map to `category`/`subcategory` for interoperability (see fault_reason_crosswalk.json). Set `codeNamespace` where the code's authority matters. |
...
| `consumerCategory` | `URBAN` / `RURAL` / `AGRICULTURE` / `INDUSTRIAL` / `MIXED` / `OTHER` | Consumer mix on the asset (local enum; a DISCOM "Feeder Type"). |
...
| `detectionRef` | Identifier | Reference to the real-time detection record that raised the outage (e.g. a DISCOM's RTDAS_DATA_ID). Absence or a "0" sentinel indicates a manually entered outage (source=MANUAL). |
| `signal` | OutageSignal | — |
