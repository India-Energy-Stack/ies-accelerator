...
| `customerRefs` | IdentifierList | — | **Optional** |
...
| `payloadDescriptorSetRef` | text | — | **Optional** — Reference ID matching a previously exchanged PayloadDescriptorProfile's payloadDescriptorSet. |
...
| `serviceDeliveryPointRefs` | IdentifierList | — | **Optional** |
| `payloadDescriptorSetRef` | text | — | **Optional** — Reference ID matching a previously exchanged PayloadDescriptorProfile's payloadDescriptorSet. |
| `compactSequenceRef` | text | — | **Optional** — Name of the compact sequence to use from the payloadDescriptorSets. |
| `intervalPeriod` | IntervalPeriod | — | **Mandatory** |
| `intervals` | list of Interval | — | **Optional** |
| `readings` | list of Reading | — | **Optional** |
...
| `serviceDeliveryPointRefs` | IdentifierList | — | **Optional** |
| `payloadDescriptorSetRef` | text | — | **Optional** — Reference ID matching a previously exchanged PayloadDescriptorProfile's payloadDescriptorSet. |
| `timePeriod` | TimePeriod | — | **Mandatory** |
| `readings` | list of Reading | — | **Mandatory** |
...
| `amountDue` | number | — | **Mandatory** |
| `energyCharges` | number | — | **Optional** — Charges for active/reactive energy consumption. |
| `fixedCharges` | number | — | **Optional** — Fixed charges/demand charges. |
| `otherCharges` | number | — | **Optional** — Other taxes, duties, surcharges, or adjustments. |
| `prepaidBalance` | number | — | **Optional** — Prepaid remaining balance/credit amount on the account/meter, if applicable. |
| `paymentStatus` | text | — | **Optional** — Status of the payment, e.g. PAID, UNPAID, PARTIAL. |
...
| `payloadDescriptorSetRef` | text | — | **Optional** — Reference ID matching a previously exchanged PayloadDescriptorProfile's payloadDescriptorSet. |
| `timestamp` | date-time | — | **Mandatory** |
| `alarms` | list of MeterAlarm | — | **Mandatory** |
...
| Field | Type | Standard | Description |
|---|---|---|---|
| `readingType` | text | — | **Mandatory** |
| `supportedModes` | list of READING / USAGE | — | **Mandatory** |
| `defaultMode` | READING / USAGE | — | **Mandatory** |
| `name` | text | — | **Optional** |
| `shortLabel` | text | — | **Optional** |
| `unit` | kWh / kVAh / kvarh / kW / kvar / kVA / V / A / Hz / PF / NONE / INR / USD | — | **Optional** |
| `phase` | NONE / R / Y / B / ABC | — | **Optional** |
| `flowDirection` | IMPORT / EXPORT / NONE | — | **Optional** |
...
| `compactSequences` | list of CompactSequence | — | **Optional** |
...
|---|---|---|---|
| `name` | text | — | **Mandatory** |
| `sequenceItems` | list of SequenceItem | — | **Mandatory** |
...
|---|---|---|---|
| `readingType` | text | — | **Mandatory** |
| `attribute` | value / occurredAt / openingValue / closingValue / validationStatus | — | **Optional** |
...
|---|---|---|---|
| `readingType` | text | — | **Mandatory** |
| `obis` | text | — | **Optional** — Optional canonical OBIS code when readingType uses a short code. |
| `name` | text | — | **Optional** |
| `unit` | kWh / kVAh / kvarh / kW / kvar / kVA / V / A / Hz / PF / NONE / INR / USD | — | **Optional** |
| `flowDirection` | IMPORT / EXPORT / NONE | — | **Optional** |
| `category` | text | — | **Optional** |
| `reportedMode` | READING / USAGE | — | **Optional** |
| `touZone` | integer | — | **Optional** |
| `multiplier` | number | — | **Optional** — Decimal scaling factor (e.g. 0.001 for milli, 1000 for kilo). Default value is 1. |
| `accuracy` | number | — | **Optional** — Accuracy class or precision value, applied after the multiplier. |
...
| `intervalPeriod` | IntervalPeriod | — | **Optional** |
| `payloads` | list of number / string / boolean | — | **Optional** |
| `readings` | list of Reading | — | **Optional** |
| `overrides` | list of Override | — | **Optional** |

### Override

| Field | Type | Standard | Description |
|---|---|---|---|
| `descriptorIndex` | integer | — | **Mandatory** |
| `occurredAt` | date-time | — | **Optional** |
| `zone` | integer | — | **Optional** |
| `validationStatus` | VALID / ESTIMATED / MANUAL / SUSPECT / REJECTED | — | **Optional** |
| `source` | METER / HES / ESTIMATED / MANUAL / IMPORT / MDM_COMPUTED / CIS_COMPUTED | — | **Optional** |
...
| `openingValue` | number | — | **Optional** — MUST ONLY be provided when the associated payload descriptor specifies reportedMode=USAGE. |
| `closingValue` | number | — | **Optional** — MUST ONLY be provided when the associated payload descriptor specifies reportedMode=USAGE. |
| `integrationPeriod` | duration | — | **Optional** — Demand integration period, e.g. PT30M or PT15M. |
...
| `paymentMode` | PREPAID / POSTPAID | — | **Optional** — Indicates if the connection is prepaid or postpaid. |
| `connectionType` | Single-phase / Three-phase | — | **Optional** — Type of connection (Single-phase or Three-phase). |
| `contractMaxDemandKw` | number | — | **Optional** — Maximum demand contracted with the utility for this connection, in kW. |
| `tariffCategoryCode` | text | — | **Optional** — Billing/tariff category code assigned by the utility. |
| `sanctionedExportLoadKw` | number | — | **Optional** — Sanctioned/approved grid export limit in kW. |
...
|---|---|---|---|
| `id` | Identifier | — | **Mandatory** |
| `make` | text | — | **Optional** — Make of the meter. |
| `model` | text | — | **Optional** — Model of the meter. |
| `meterType` | AMR / AMI / Electromechanical / Forward / Reverse / Bidirectional / Prepaid / NetMeter / Other | — | **Optional** |
...
| `meterRefs` | IdentifierList | — | **Mandatory** |
| `parentResources` | list of Identifier | — | **Optional** — List of parent resources (such as feeders or DTs) for this association, replacing feederId and dtId. |
| `telemetryProvider` | text | — | **Optional** — Telemetry provider for this meter-service association. |
| `commissioningDate` | date-time | — | **Optional** — Commissioning date of the meter association. |
| `generationCapacityKw` | number | — | **Optional** — Rated power generation capacity (e.g. solar PV inverter capacity) in kW. |
| `storageCapacityKw` | number | — | **Optional** — Rated energy storage capacity in kWh. |
...
|---|---|---|---|
| `timestamp` | date-time | — | **Mandatory** |
| `alarmId` | integer | — | **Mandatory** |
| `alarmName` | text | — | **Optional** |
| `status` | ACTIVE / CLEARED | — | **Mandatory** |
...
|---|---|---|---|
| `profileType` | text | — | **Mandatory** |
| `id` | text | — | **Mandatory** — Unique identifier for this specific configuration state. |
| `payloadDescriptorSets` | list of PayloadDescriptorSet | — | **Mandatory** |

### CustomerDetails

| Field | Type | Standard | Description |
|---|---|---|---|
| `name` | text | — | **Optional** — Full name of the customer as per ID proof. |
