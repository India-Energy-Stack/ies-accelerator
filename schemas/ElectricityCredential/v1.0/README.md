...
|---|---|---|---|---|
| `id` | uri | — | Mandatory | Unique identifier for the credential in URN UUID format |
| `type` | list of text | — | Mandatory | — |
| `issuer` | object | — | Mandatory | — |
| `validFrom` | date-time | — | Mandatory | Date and time from which the credential is valid (W3C VC Data Model 2.0) |
| `validUntil` | date-time | — | Optional | Date and time until which the credential is valid (W3C VC Data Model 2.0) |
| `credentialStatus` | object | — | Optional | Revocation status information for the credential |
| `credentialSubject` | object | — | Mandatory | Subject with profile sections: customerProfile (required), customerDetails, consumptionProfiles, generationProfiles, storageProfiles. |
...
| Field | Type | Standard | Status | Description |
|---|---|---|---|---|
| `customerNumber` | text | — | Mandatory | Full customer account number assigned by the utility |
| `meterNumber` | text | — | Mandatory | Unique meter serial number |
| `meterType` | AMR / AMI / Electromechanical / Forward / Reverse / Bidirectional / Prepaid / NetMeter / Other | — | Mandatory | Type of electricity meter installed. Values derived from Green Button / ESPI meter kind classifications. |
| `idRef` | object | — | Optional | Reference to an identity issued by an external authority |

### CustomerDetails

| Field | Type | Standard | Status | Description |
|---|---|---|---|---|
| `fullName` | text | — | Mandatory | Full name of the customer as per ID proof |
| `installationAddress` | object | — | Mandatory | The physical location of the installation. Follows the beckn Location schema. |
| `serviceConnectionDate` | date-time | — | Mandatory | Date and time when the electricity connection was activated (with timezone offset) |

### ConsumptionProfile

| Field | Type | Standard | Status | Description |
|---|---|---|---|---|
| `id` | uri | — | Mandatory | DID of the customer/credential subject (links to customer) |
| `consumerNumber` | text | — | Mandatory | Full consumer account number assigned by the utility |
| `fullName` | text | — | Mandatory | Consumer name |
| `premisesType` | Residential / Commercial / Industrial / Agricultural | — | Mandatory | Type of premises for the electricity connection |
| `connectionType` | Single-phase / Three-phase | — | Mandatory | Type of electrical connection |
| `sanctionedLoadKW` | number | — | Mandatory | Sanctioned/approved electrical load in kilowatts (kW) |
| `tariffCategoryCode` | text | — | Mandatory | Billing/tariff category code assigned by the utility |
| `meterNumber` | text | — | Optional | Meter serial number (optional, for linking to specific meter) |
...
|---|---|---|---|---|
| `id` | uri | — | Mandatory | DID of the customer/credential subject (links to customer) |
| `consumerNumber` | text | — | Mandatory | Consumer account number assigned by the utility |
| `fullName` | text | — | Optional | Consumer name (optional) |
| `meterNumber` | text | — | Optional | Meter serial number associated with this generation asset (optional) |
| `assetId` | text | — | Optional | Unique identifier for the generation asset |
| `generationType` | Solar / Wind / MicroHydro / Other | — | Mandatory | Type of distributed energy generation |
| `capacityKW` | number | — | Mandatory | Installed generation capacity in kilowatts (kW) |
| `commissioningDate` | date | — | Mandatory | Date when the generation system was activated |
| `manufacturer` | text | — | Optional | Equipment manufacturer |
| `modelNumber` | text | — | Optional | Equipment model number |
...
| `consumerNumber` | text | — | Mandatory | Consumer account number assigned by the utility |
| `fullName` | text | — | Optional | Consumer name (optional) |
| `meterNumber` | text | — | Optional | Meter serial number associated with this storage asset (optional) |
| `assetId` | text | — | Optional | Unique identifier for the storage asset |
| `storageCapacityKWh` | number | — | Mandatory | Battery storage capacity in kilowatt-hours (kWh) |
| `powerRatingKW` | number | — | Mandatory | Battery charge/discharge power rating in kilowatts (kW) |
| `commissioningDate` | date | — | Mandatory | Date when the storage system was activated |
| `storageType` | LithiumIon / LeadAcid / FlowBattery / Other | — | Optional | Type of battery storage technology |
