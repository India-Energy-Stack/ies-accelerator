...
- **What a provider can serve** (`MeterDataCapabilities`) — the profile types it supports (`CustomerProfile`, `IntervalProfile`, `DailyProfile`, `MonthlyProfile`, `BillDetails`, `InstantaneousProfile`, `EventProfile`, `AlarmProfile`), optionally down to the specific registers or readings within each profile (`ProfileCapability.readings[]`), which query scopes it can answer (`supportedScopes[]`), and the longest historical window it can serve (`maxHistoryDuration`). The only required field on `MeterDataCapabilities` is `profiles[]` — a provider can publish a capability set with no stated scope or history limit, though in practice the real examples always fill both in.
- **Who has been authorised, for what, and for how long** (`MeterDataAuthorisation`) — a grant naming the authorising entity (`grantor`), the authorised entity (`grantee`), the `purpose` of access as free text (e.g. "ESG Carbon Accounting & Trend Analysis" in the shipped example), a `validFrom`/`validUntil` validity window, and the specific slice of `capabilities` being granted. All six fields — `grantor`, `grantee`, `purpose`, `validFrom`, `validUntil`, `capabilities` — are required; there is no way to issue a partial or open-ended authorisation.
- **The actual query being asked** (`MeterDataRequestObject`, the JSON Schema definition backing the `MeterDataRequest` type name) — which `consumers[]` or `resources[]` it targets (both optional and independent — a request can target resources without naming consumers, or vice versa), whether the request covers just that resource or its children too (`scope`), the `from` start time and `duration` length of the requested window, `consumerConsent[]` references to consumer consent, the `authorisation` backing the request, the `capabilitiesRequested` slice being asked for, and an optional `maxRecordsShared` cap on how many records come back. Only `from`, `duration` and `capabilitiesRequested` are required — a query can omit `consumers`, `resources`, `scope`, `consumerConsent` and `maxRecordsShared` entirely.

A short real example makes the capability slice concrete. The shipped `Anonymised_Telemetry_Request.json` asks for two BESCOM meters' interval telemetry without naming any consumer at all:

```json
"resources": [
  "did:dedi:bescom:meters:BESCOM-SM-2025-654321",
  "did:dedi:bescom:meters:BESCOM-SM-2025-999999"
],
"scope": "ResourceOnly",
...
The schema targets consumers and resources by URI/DID: `consumers[]` is a list of consumer URIs or DIDs, and `resources[]` is a list of resource URIs or DIDs — for example meter DIDs or service-point URIs. The real examples use the `did:dedi:` method throughout — `did:dedi:ies:customer:IN-MH-CUST-80`, `did:dedi:ies:meter:IN-MH-MTR-89721`, `did:dedi:ies:discom:BESCOM`, `did:dedi:ies:tsp:ACME-TSP` — showing a consistent `did:dedi:ies:<role>:<local-id>` shape, though the schema itself only constrains these fields to `format: uri` and does not mandate the `did:dedi` method. `MeterDataAuthorisation.grantor` and `.grantee` are likewise URIs/DIDs identifying the authorising and authorised entities. `consumerConsent[]` is looser still — plain `text` strings rather than `uri`-formatted, and the example payload uses a consent identifier (`did:dedi:ies:consent:CN-89021-SIGNED-BY-USER`) rather than a hash, though the field description only calls for "consent references/receipts."
...
- **MeterDataAuthorisation** — a grant record with a defined validity window (`validFrom`/`validUntil`). It is issued by the grantor (e.g. the DISCOM) to a grantee (e.g. a TSP) for a stated purpose, and it names a specific `MeterDataCapabilities` subset the grantee is allowed to draw on. It changes whenever access is newly granted, narrowed, or expires. The shipped `Authorisation_Example.json` is concrete about what a real grant narrows down to: BESCOM (`did:dedi:ies:discom:BESCOM`) grants ACME-TSP (`did:dedi:ies:tsp:ACME-TSP`) a one-year window (`2026-05-01` to `2027-05-01`) for "ESG Carbon Accounting & Trend Analysis" — restricted to just two registers (`kWh imp block` under `IntervalProfile`, `kWh imp` under `DailyProfile`), a single scope (`ResourceOnly`), and a one-year history cap (`P1Y`) — visibly narrower than the provider's full advertised capability set.
