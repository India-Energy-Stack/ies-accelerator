...
MeterDataCredential stands on its own: it is issued, held and verified against the provider's published key, whatever channel carries it. The provider (an AMISP, MDM system, or DISCOM) issues the credential wrapping the requested telemetry — binding the data to its own identity, a validity window, and a revocation path through the DeDi registry — and delivers it to the receiver or into the consumer's wallet. On the request side, its counterpart is the **MeterDataRequestCredential**, which proves the requester's right to ask; MeterDataCredential attests what was actually delivered.
