...
# Relative column widths for the PDF (pandoc reads the dash counts): a narrow
# Field, a wider Type, and the widest column for Description. GitHub ignores
# these and auto-sizes.
COL_RULE = "|----|-------|-----------------|"


def _code_enum(values) -> str:
    """Join enum/unit values, each in a code span so they render in the same
    monospace as the Field column (e.g. "`AMR` / `AMI`")."""
...
def type_str(prop: dict, defs: dict | None = None) -> str:
    ...
    if r:
        ...
        if isinstance(target, dict):
            # Inline a named enum's values (e.g. Phase -> "`NONE` / `R` / `Y` / …").
            if "enum" in target and "properties" not in target:
                return _code_enum(target["enum"])
...
def component_table(title: str, comp: dict, defs: dict) -> str:
    ...
    if not rows:
        return ""
    out = [f"### {title}", "", "| Field | Type | Description |", COL_RULE]
