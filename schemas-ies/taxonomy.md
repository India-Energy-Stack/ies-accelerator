...
| actualRestoration | string | Set when status=RESTORED; feeds IEEE 1366 indices. | ON | IEEE 1366 |
| address † | Address | Postal address of the service delivery point (Address object). | EC, MD | schema.org PostalAddress |
...
| alarmId † | integer | Numeric identifier of the alarm raised by the meter or head-end system. | MD, ON | IS 15959 |
| alarmName † | string | Human-readable name of the alarm. | MD | IS 15959 |
| alarmRefs | array | References into MeterData AlarmProfile records. | ON | — |
| alarms † | array | Alarm records carried in an ALARM profile (MeterAlarm entries). | MD | IS 15959 |
...
| applicationProtocol | string | Application-layer protocol for meter data. DLMS_COSEM: IEC 62056, mandatory for India AMI per BIS IS 16444. ANSI_C12_18: North American. Orthogonal to communicationTechnology (physical layer). | EC | IS 16444 |
...
| bbox | array | Optional bounding box `[west, south, east, north]` in degrees. | EC | GeoJSON RFC 7946 |
...
| connectorType | string | Physical connector standard. Type1: IEC 62196-2 Type 1 (J1772). Type2: IEC 62196-2 Type 2 (Mennekes). CCS1/CCS2: Combined Charging System DC fast charge. CHAdeMO: CHAdeMO DC fast charge. GB_T: GB/T 20234. NACS: SAE J3400. | EC | IEC 62196-2; CCS1/CCS2; CHAdeMO; GB/T 20234; SAE J3400 |
...
| controllable | boolean | True if the aggregator is authorised to issue dispatch / curtailment instructions. False = observation-only. | EC | IEEE 2030.5; IEC 61850-7-420 |
...
| controlProtocol | string | Demand-response / control protocol supported by this load device. | EC | OpenADR 2.0b; OCPP 2.0.1; SunSpec Modbus; EEBus |
| coordinates | array | Coordinates per RFC 7946 for all types **except** GeometryCollection. Order is **[lon, lat, (alt)]**. For Polygons, this is an array of linear rings; each ring is an array of positions. | EC | GeoJSON RFC 7946 |
...
| customersAffected † | integer | Number of consumers affected by the outage. | ON | IEEE 1366 |
| date | string | ISO 8601 date the inspection was performed. | EC | CEA Connectivity Regs 2013; IEEE 1547-2018 Cl.11 |
...
| geometries | array | Member geometries when `type` is **GeometryCollection**. | EC | GeoJSON RFC 7946 |
...
| headline † | string | Short public-facing headline of the outage notice. | ON | OASIS CAP v1.2 |
...
| impact † | OutageImpact | Impact of the outage (OutageImpact: customers affected, assets, area). | ON | IEC 61968-3 (CIM); IEEE 1366 |
...
| inspectorId | string | Identifier of the inspector or inspecting body, as recorded by the licensee. | EC | CEA Connectivity Regs 2013; IEEE 1547-2018 Cl.11 |
...
| instruction | string | What the consumer should do. | ON | OASIS CAP v1.2 |
| integrationPeriod | string | Demand integration period, e.g. PT30M or PT15M. | MD | — |
| intervalPeriod | IntervalPeriod | IntervalPeriod | MD | IS 15959 / DLMS-COSEM |
| intervals | array | list of Interval | MD | IS 15959 / DLMS-COSEM |
...
| language | string | BCP-47, e.g. en, hi. | ON | BCP-47 |
...
| magnitude † | number | Measured magnitude associated with the event (e.g. voltage during a sag). | MD | IS 15959 |
...
| meterCategory | MeterCategory | A / B / C / D1 / D2 / D3 / D4 | MD | IS 15959 |
...
| obis | string | Optional canonical OBIS code when readingType uses a short code. | MD | IS 15959 |
...
| postalCode | string | Postal/ZIP code. | EC | schema.org PostalAddress |
...
| publicInfo | OutagePublicInfo | OutagePublicInfo | ON | OASIS CAP v1.2; BCP-47 |
...
| result | string | Inspection outcome. 'conditional' indicates pass subject to remedial action. | EC | CEA Connectivity Regs 2013; IEEE 1547-2018 Cl.11 |
...
| sequence † | integer | Sequence number of the event within its timestamp. | MD | IS 15959 |
...
| streetAddress | string | Street address (building name/number and street). | EC | schema.org PostalAddress |
...
| text | string | Free-text localities (CAP areaDesc), e.g. "SEC-14, 9, 11 Rajnagar". | ON | OASIS CAP v1.2 (areaDesc) |
...
| touBuckets | array | list of TouBucket | MD | IS 15959 / DLMS-COSEM |
| touZone † | integer | Time-of-use zone number the reading belongs to. | MD | IS 15959 |
| type | array; string | Asset-class discriminator. DT → PowerTransformer, BUS → BusbarSection, FEEDER → Feeder (EquipmentContainer), MICROGRID → Substation / microgrid container (IEC 61970-301). | EC | IEC 61970-301 |
...
| v2xProtocol | string | Vehicle-to-Grid / V2X protocol. Present only for EV_V2G resources. | EC | ISO 15118-20; OCPP 2.1 BPT |
...
| Address | Address; object | **Postal address** aligned with schema.org `PostalAddress`. Use for human-readable addresses. Geometry lives in `Location.geo` as GeoJSON. | EC, MD | schema.org PostalAddress |
| AlarmProfile | object | Real-time active indicators/alerts representing immediate state conditions from the meter. | MD | IS 15959 |
...
| EnergyResourceEVCharger | EnergyResourceCommon | An EV charging station (EVSE) energy resource. EV_V2G adds bidirectional Vehicle-to-Grid capability per ISO 15118-20 / OCPP 2.1 BPT. CIM: ElectricVehicleChargingStation (CIM17+). | EC | CIM ElectricVehicleChargingStation (CIM17+) |
...
| EnergyResourceGenerator | EnergyResourceCommon | A generation DER energy resource (SOLAR_PV, WIND, HYDRO, BIOGAS, CHP, FUEL_CELL). CIM: GeneratingUnit subtypes (IEC 61970-301). | EC | CIM GeneratingUnit (IEC 61970-301) |
...
| EnergyResourceInverter | EnergyResourceCommon | A grid-connected power-electronics inverter energy resource. Captures IEEE 1547-2018 ride-through categories, operating mode, and reactive / frequency-support capabilities. CIM: PowerElectronicsConnection (IEC 61970-302). | EC | CIM PowerElectronicsConnection (IEC 61970-302) |
...
| EnergyResourceLoad | EnergyResourceCommon | A controllable load energy resource (smart HVAC, smart water heater, or generic controllable load). CIM: EnergyConsumer / ConformLoad (IEC 61970-301). | EC | CIM EnergyConsumer/ConformLoad (IEC 61970-301) |
...
| EnergyResourceMeter | EnergyResourceCommon | A metering-point energy resource. Anchors all DER sub-resources behind it in the topology tree. CIM: cim:Meter (IEC 61968-9). | EC | CIM cim:Meter (IEC 61968-9) |
...
| EnergyResourceNetwork | EnergyResourceCommon | A grid-network infrastructure energy resource (distribution transformer, busbar, feeder, or microgrid). Topology containers that anchor metering points and DER resources. CIM: PowerTransformer, BusbarSection, Feeder, Substation (IEC 61970-301). | EC | CIM PowerTransformer/BusbarSection/Feeder/Substation (IEC 61970-301) |
...
| EnergyResourceStorage | EnergyResourceCommon | A stationary battery energy storage resource (BESS). CIM: BatteryUnit (IEC 61970-302). | EC | CIM BatteryUnit (IEC 61970-302) |
...
| InstantaneousProfile | object | Snapshot of a meter's electrical quantities at one captured moment. | MD | IS 15959 |
| Interval | object | One time-series interval row. payloads[k] is the value for compactSequence[k]. | MD | IS 15959 / DLMS-COSEM |
| IntervalPeriod | object | Period defined by a start time and a duration. | MD | IS 15959 / DLMS-COSEM |
| IntervalProfile | object | Block Load Survey — PT15M / PT30M cadence intervalBlocks for one meter. | MD | IS 15959 / DLMS-COSEM |
...
| MeterAlarm † | object | Single alarm record: timestamp, alarmId, alarmName, status, severity. | MD | IS 15959 |
...
| MeterEvent † | object | Single event record: timestamp, eventId, eventName, phase, sequence, magnitude, duration. | MD | IS 15959 |
| MeterType | string | Type of electricity meter. Shared with ElectricityCredential schema. | MD | — |
| MonthlyProfile | object | Monthly profile readings from the meter at billing reset time (billing history). | MD | IS 15959 / DLMS-COSEM |
| OutageAffectedArea | object | CAP area block — free text plus structured admin areas plus GIS geometry. | ON | OASIS CAP v1.2; GeoJSON RFC 7946 |
...
| OutageImpact | object | CIM-aligned affected service points and customer counts (feeds SAIDI/SAIFI). | ON | IEC 61968-3 (CIM); IEEE 1366 |
...
| OutagePublicInfo | object | CAP info block — human-readable text for web publishing and push. | ON | OASIS CAP v1.2; BCP-47 |
...
| QVApparentPower | object | Apparent-power quantity {value, unit}. unit is a QUDT apparent-power alias (kVA, MVA) expanded to a QUDT IRI via the JSON-LD context. | EC | QUDT |
| QVEnergy | object | Energy quantity {value, unit}. unit is a QUDT energy alias (kWh, MWh) expanded to a QUDT IRI via the JSON-LD context. | EC | QUDT |
| QVPower | object | Active-power quantity {value, unit}. unit is a QUDT power alias (W, kW, MW) expanded to a QUDT IRI via the JSON-LD context. | EC | QUDT |
| QVReactivePower | object | Reactive-power quantity {value, unit}. unit is a QUDT reactive-power alias (kVAR, MVAR) expanded to a QUDT IRI via the JSON-LD context. value may be negative (absorption). | EC | QUDT |
| QVVoltage | object | Voltage quantity {value, unit}. unit is a QUDT voltage alias (V, kV) expanded to a QUDT IRI via the JSON-LD context. | EC | QUDT |
| Reading | object | Detailed, self-contained inline reading with full metadata support. | MD | IS 15959 / OBIS |
...
| TouBucket | object | Time of Use bucket separating readings by zone. | MD | IS 15959 / DLMS-COSEM |
...
| ValueCapability | object | Declares capabilities for a specific register/reading within a profile. | MDR | IS 15959 / IEC 62056 (OBIS) |
