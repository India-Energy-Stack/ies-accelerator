...
| **Status** | Live in pilot (APEPDCL, DGVCL, Tata Power; PVVNL staged) |
...
|---|---|---|
| DISCOM (issuer) | `did:web` on owned domain | `did:web:ies.apepdcl.in` |
| Regulator | `did:web` on owned domain | `did:web:ies.aperc.gov.in` |
| Consumer (holder) | `did:key` (wallet) or `tel:` URI | `did:key:z6MkjVQ8r4f3rPuY…` |
| Meter / inverter / DER / transformer | `did:web` under the issuer domain, with path | `did:web:ies.apepdcl.in:assets:meter:NM-44091234` |
