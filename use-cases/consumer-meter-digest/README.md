...
| Block | Used for |
|---|---|
| [Identifiers and Addressing](../../what-ies-provides/identifiers/README.md) | DISCOM's `did:web`; consumer's wallet `did:key`; meter and connection DIDs that anchor the Digest |
...
| [Data Exchange](../../what-ies-provides/data-exchange/README.md) | If the consumer-pull endpoint is fronted by your BPP over Beckn, the BAP/BPP machinery is the same as [Smart Meter Data Exchange](../smart-meter-data-exchange/README.md) — only the trigger (consumer, not DISCOM) and the `validUntil` differ |
| [DigiLocker delivery](../../what-ies-provides/energy-credentials/digilocker.md) | The dominant delivery channel into the consumer's wallet |
...
3. **Exchange.** Issue the Digest via [Energy Credentials — Issue your first credential](../../what-ies-provides/energy-credentials/README.md#issue-your-first-credential), setting `credentialSubject.id` to the consumer's wallet DID, `schemaId` to `MeterDataCredential/v0.6`, and `validUntil` to a short window matching the use case (24h for loan portals, up to 7d for non-time-sensitive flows).
