...
- [ ] **2.c** Regulator's licensing pointer obtained — the regulator's `did:web` (for `issuer.idRef.issuedBy`) and your regulator-issued licence identifier (for `issuer.idRef.subjectId`). This is the trust anchor verifiers consult; no IES-side DISCOM registry entry is required to issue credentials. (Beckn data exchange separately requires a DISCOM-reference-registry entry — see [Identifiers and Addressing → Appendix E](../identifiers/README.md#appendix-e--joining-a-beckn-network-subscriber-registry-on-the-beckn-fabric).)
- [ ] **2.d** Issuer object enrichment configured in integration service — `issuer.name` and `issuer.idRef` (`issuedBy: <regulator-did-web>`, `subjectId: <regulator-domain>:<discom-licence-id>`) injected on egress, since OpenCred only emits the DID string
