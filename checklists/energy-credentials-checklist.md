...
- [ ] **2.c** Regulator's licensing pointer obtained — the regulator's `did:web` (for `issuer.idRef.issuedBy`) and your regulator-issued licence identifier (for `issuer.idRef.subjectId`). This is the trust anchor verifiers consult; no IES-side DISCOM registry entry is required to issue credentials. (Joining the inter-DISCOM data exchange network separately requires a DISCOM-reference-registry entry — see [Identifiers and Addressing → Appendix E](../identifiers/README.md#appendix-e--joining-a-beckn-network-subscriber-registry-on-the-beckn-fabric).)
