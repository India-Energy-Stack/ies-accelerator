...
| **Any organisation joining a Beckn network** (data exchange, P2P trading — DISCOMs, AMISPs, trading platforms, aggregators) | §1.1 – §1.4, then §1.5 – §1.7 | [Setup Discovery](setup-discovery.md) |
...
- **`signingPrivateKey`** — store in your secret manager; ONIX loads it to sign outgoing messages ([Setup Discovery §3.3](setup-discovery.md#id-3.3-swap-in-your-real-identity)). Never commit it.
...
**Prerequisite:** verified namespace (§1.4), Ed25519 public key (§1.5), and the public HTTPS URL where your Beckn adapter will receive messages (you can publish first and deploy the adapter in [Setup Discovery](setup-discovery.md); the URL just has to be the one you will use).
...
2. **Add a record** with these fields:
   ...
   If you operate in both roles (BAP *and* BPP), publish a separate record per role. That `did:web` `subscriber_id` is what you set as `networkParticipant` in your ONIX config ([Setup Discovery §3.3](setup-discovery.md#id-3.3-swap-in-your-real-identity)).

3. **Note the record ID** DeDi assigns — you will configure it into ONIX as the `keyId` in [Setup Discovery §3.3](setup-discovery.md#id-3.3-swap-in-your-real-identity).
...
When your role's boxes are ticked, you have completed **Setup Register**. Credential issuers: continue to **[Issue Credentials](issue-credentials.md)**. Beckn participants: continue to **[Setup Discovery](setup-discovery.md)**.
